  ...
  async processVideo(
    ...
    try {
      ...
      try {
       ...
       - 重要なトピックの変化や話者の切り替わりポイントでタイムスタンプを設定
- 各タイムスタンプは15〜30秒以上間隔を空ける（細かすぎるタイムスタンプは避ける）
        - 全体で5〜8個の主要なタイムスタンプを作成（多すぎないこと）
        - 最初のセクションは必ず00:00:00から始めてください
        - 各タイムスタンプのタイトルは10〜15文字程度の簡潔な要約
        - 出力はJSON形式のみにしてください
        `;
      ...
      switch (step) {
        ...
        case 3: // タイムスタンプ抽出からやり直し
          ...
          try {
            ...
            // タイムスタンプ抽出用のプロンプトを作成
            const timestampPrompt = `
あなたはプロの動画編集者で、高品質なタイムスタンプを作成する専門家です。以下の文字起こしから、視聴者が探しやすい効果的なタイムスタンプを作成してください。
