...
async function processSummaryJob(job) {
  try {
    ...
    if (!record.transcript_text) {
      throw new Error(`文字起こしテキストがありません: ${recordId}`);
    }
    ...
    // 進捗状況を更新
    await queueManager.updateJobProgress(job, 30, '処理中', '要約を生成しています');
    
    // 要約を生成
    const summary = await generateSummary(record.transcript_text);
